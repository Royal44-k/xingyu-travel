...
12. The URL-bar screenshot exposed a P2 brand defect: `metadata.icons` explicitly reused a rainy-mountain photograph, so the public URL appeared with an indistinct photo thumbnail. A dedicated three-color XINGYU mark was generated, simplified to a bold mountain/path/sun silhouette, normalized to exact brand colors, and inspected at 16/32/64/128 px. `favicon-comparison-board.png` places the supplied source capture and the actual 24/32 px output in one comparison input. The new mark is distinct at browser size, contains no text or photographic detail, and introduces no actionable P0/P1/P2 issue.
...
- The dedicated browser identity used exact source commit `13d3f243d28c02168482ebc01240e4385cd1fd95`. Protected Preview `dpl_Ckz5XWR8VZurz94bMw2dQWjhMw3B` passed the complete 9-route/8-flow/4-loop matrix and focused favicon/Apple/PWA/manifest verification. The exact artifact was promoted to public Production `dpl_G15kfgFe28YqSxScLcqC67HwVNT8`; fresh unauthenticated Chrome repeated both matrices with zero runtime issues, all five icon assets returned 200 with correct formats/dimensions, and no published icon link referenced the former photograph. The error-log scan returned no entry and Preview-only SSO remained unchanged.
...
- [x] The URL-bar photograph favicon is replaced by a dedicated XINGYU mark verified at browser and installable-app sizes.
